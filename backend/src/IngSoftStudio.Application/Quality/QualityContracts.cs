...
public sealed record CreateRiskRequest(
    ...
    [property: Required, StringLength(2000, MinimumLength = 2)] string Description,
    [property: EnumDataType(typeof(RiskProbability))] RiskProbability Probability,
    [property: EnumDataType(typeof(RiskImpact))] RiskImpact Impact,
    [property: Required, StringLength(2000, MinimumLength = 2)] string Mitigation);
...
public sealed record ChangeRiskStatusRequest(
    [property: EnumDataType(typeof(RiskStatus))] RiskStatus Status);

public sealed record CreateTestCaseRequest(
    Guid? RequirementId,
    [property: Required, StringLength(200, MinimumLength = 2)] string Title,
    [property: Required, StringLength(2000)] string Preconditions,
    [property: Required, StringLength(4000, MinimumLength = 2)] string Steps,
    [property: Required, StringLength(2000, MinimumLength = 2)] string ExpectedResult);

public sealed record ExecuteTestCaseRequest(
    [property: EnumDataType(typeof(TestCaseStatus))] TestCaseStatus Status,
    [property: StringLength(2000)] string? ActualResult);
...
public sealed record CreateDefectRequest(
    ...
    [property: EnumDataType(typeof(DefectSeverity))] DefectSeverity Severity,
    [property: EnumDataType(typeof(DefectPriority))] DefectPriority Priority);

public sealed record ChangeDefectStatusRequest(
    [property: EnumDataType(typeof(DefectStatus))] DefectStatus Status);
